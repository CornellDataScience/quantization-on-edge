...
def quantize_cnn(prep_model_path, quantized_params_path, quantized_activations_path, quantized_biases_path, output_model_path):
    '''
    Quantize ONNX CNN model (using symmetric quantization scheme) and save quantized model

    Input
    -----
    prep_model_path: file path of ONNX CNN model to quantize
    quantized_params_path: file path of quantized parameter scalar and zero point values used to quantize
    quantized_activations_path: file path of scalar and zero point values for prepped model activations
    quantized_biases_path: file path of quantized bias, scalar, and zero point values used to quantize
    output_model_path: file path to save quantized model to
    ...
    for i,node in enumerate(graph_nodes):
        ...
        if (node.op_type=="Conv" and i+2 < len(graph_nodes)
            and graph_nodes[i+1].op_type=="BatchNormalization"
            and graph_nodes[i+2].op_type=="Relu"
            and graph_nodes[i+1].input[0]==node.output[0]
            and graph_nodes[i+2].input[0]==graph_nodes[i+1].output[0]):
            conv, bn, relu = node, graph_nodes[i+1], graph_nodes[i+2]
            X, W = conv.input[0], conv.input[1]
            B = conv.input[2] if len(conv.input)>2 else ""
            scale, bias_p, mean, var = bn.input[1:]
            eps_val = get_eps(bn)            
            eps_name = f"{conv.name}_eps"  
            eps_init = numpy_helper.from_array(
                np.array([eps_val],dtype=np.float32), eps_name
            )                             
            new_initializers.append(eps_init) 
            fused = helper.make_node(
                "ConvBNReLUFusion", inputs=[X,W,B,scale,bias_p,mean,var,eps_name],
                outputs=relu.output, name=conv.name+"_Fused", domain="ai.onnx.contrib"
            )                                
            added_nodes.append(fused)
            removed_nodes.extend([conv,bn,relu])
            activation_initializers.add(eps_name)
            continue 
