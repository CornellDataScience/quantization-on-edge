...
def quantize_biases_cnn(prep_model_path, quantized_activations_path, quantized_params_path, output_quantized_biases_path):
    '''
    Quantize CNN model biases stored in a JSON file and save the quantized biases to another JSON file.
    Input
    -----
    onnx_model_path: ONNX model to extract biases from
    input_path : file path to the input JSON file containing quantized parameters
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        ...
        if mode == "symmetric":
            ...
            quantize_biases_cnn(prep_model_path, quantized_activations_path, quantized_params_path, output_quantized_biases_path)
